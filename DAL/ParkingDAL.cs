...
namespace ParkingSystem.DAL
{
    ...
    {
        public DataTable GetSpaces() => DbHelper.GetDataTable("SELECT SpaceNumber, IsOccupied FROM ParkingSpace ORDER BY SpaceNumber");
        ...
        public void StartParking(int vehicleId, string spaceNumber)
        {
            ...
            var occupied = Convert.ToBoolean(dt.Rows[0]["IsOccupied"].ToString());
        ...
        {
            var sql = "DELETE FROM ParkingSpace WHERE SpaceNumber = @num";
            DbHelper.ExecuteNonQuery(sql, new SqlParameter("@num", spaceNumber));
        }

        public void RemoveTicket(int ticketId)
        {
            var sql = "DELETE FROM ParkingTicket WHERE TicketID = @id";
            DbHelper.ExecuteNonQuery(sql, new SqlParameter("@id", ticketId));
        }
        ...
        public DataTable GetTickets()
        {
            return DbHelper.GetDataTable(@"
                SELECT 
                    'TICK-' + RIGHT('000' + CAST(t.TicketID AS NVARCHAR(3)), 3) AS UniqueNumber,
                    v.LicensePlate, 
                    p.SpaceNumber, 
                    t.EntryTime, 
                    t.ExitTime, 
